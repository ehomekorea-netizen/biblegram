...
async function analyzeVerseForVisuals(verse) {
  const prompt = `다음 성경 구절의 영적/신학적 분위기를 정밀 분석하여, 이에 가장 잘 어울리는 시각적 테마와 핵심 영어 신앙 개념을 생성해 주세요.
  성경 구절: ${verse}
  형식은 반드시 다음 구조의 완성된 순수 JSON 객체여야 합니다:
  {
    "visualTheme": "구절에 가장 어울리는 테마 (반드시 다음 중 하나만: cross, light, nature, water, night, mountain, sky, desert)",
...
async function generateVerseAudio(verse) {
  ...
  try {
    ...
    throw new Error("오디오 합성 API 응답 오류", { cause: e });
  }
}
              ...
              return parentComments.map(parent => (
                <div key={parent.id} className="flex flex-col gap-2.5">
                  ...
                  >
                    ...
                    <p className="text-stone-200 text-[12.5px] tracking-wide leading-relaxed mt-0.5 break-all">
                      {parent.comment_text}
                    </p>
                  ...
                  {repliesMap[parent.id] && repliesMap[parent.id].map(reply => (
                    ...
                    >
                      ...
                      <p className="text-stone-300 text-[12px] tracking-wide leading-relaxed mt-0.5 break-all">
                        {reply.comment_text}
                      </p>
                      <div className="flex items-center gap-3 mt-1">
                        <button 
    ...
    <div className="relative w-full h-full flex flex-col justify-between bg-[#030202] text-[#F9F7F1] p-8 overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col gap-4 pb-12 w-full">
        ...
        <button 
          ...
          className="w-full py-4 rounded-xl bg-transparent hover:bg-white/5 border border-white/15 text-stone-300 font-medium text-[13.5px] tracking-wide active:scale-[0.98] transition-all"
        >
          게스트 로그인
        </button>
      </div>
...
const handleSearchVerse = async () => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setLoadingStep('성경 구절의 분위기에 어울리는 성화를 거룩하게 선정하는 중...');
...
const getVisibleCardsForActiveProfile = () => {
      if (activeProfileUser === nickname || activeProfileUser === "은혜나눔인") {
        return savedCards;
      } else {
          ...
          <>
            ...
            {view === 'profile' && (
              <div className="flex-1 flex flex-col bg-[#050505] z-10 pb-16 overflow-y-auto hide-scrollbar">
                ...
                {activeProfileUser !== nickname && activeProfileUser !== "은혜나눔인" ? (
                  ...
                  <div className="pt-20 px-4 flex justify-end bg-[#111111]">
                    <button 
                      onClick={handleLogout}
                      className="flex items-center gap-1.5 text-stone-400 hover:text-[#ef4444] text-[11px] bg-white/5 border border-white/10 py-1.5 px-3.5 rounded-full transition-colors"
                    >
                      로그아웃
                    </button>
                  </div>
            ...
            <div className="absolute bottom-0 w-full h-[80px] bg-gradient-to-t from-black via-black/95 to-transparent backdrop-blur-md flex justify-around items-center pb-4 px-8 z-50 pointer-events-auto border-t border-white/[0.04]">
              ...
              <button 
                ...
                className={`flex flex-col items-center p-2.5 transition-all duration-300 ${(view === 'profile' || view === 'detail') && (activeProfileUser === nickname || activeProfileUser === '은혜나눔인') ? 'text-[#DFBA73] scale-110 drop-shadow-md' : 'text-white/30'}`}
              >
                <Icons.User />
